    ...
    ($name:ident: ( $( $in_param_name:ident: $in_param_type:ty ),* ) => ( $( $out_param_name:ident: $out_param_type:ty ),* )) => {
        ...
        paste::paste! {
            ...
            fn [<$name _impl>](&mut self, mut ctx: &mut $crate::ipc::server::ServerContext) -> $crate::result::Result<()> {
                ...
                $crate::ipc::server::write_request_command_response_on_ipc_buffer(&mut ctx.ctx, $crate::result::ResultSuccess::make());
    ...
    ($name:ident: ( $( $in_param_name:ident: $in_param_type:ty ),* ) => ( $( $out_param_name:ident: $out_param_type:ty ),* )) => {
        ...
        paste::paste! {
            ...
            fn [<$name _impl>](&mut self, mut ctx: &mut $crate::ipc::server::ServerContext) -> $crate::result::Result<()> {
                ...
                $crate::ipc::server::write_control_command_response_on_ipc_buffer(&mut ctx.ctx, $crate::result::ResultSuccess::make());
