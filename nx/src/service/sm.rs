...
impl service::ISessionObject for UserInterface {
...
impl IUserInterface for UserInterface {
    fn initialize(&mut self, process_id: sf::ProcessId) -> Result<()> {
        ipc_client_send_request_command!([self.session.session; 0] (process_id) => ())
    }

    fn get_service(&mut self, name: ServiceName) -> Result<sf::MoveHandle> {
        ipc_client_send_request_command!([self.session.session; 1] (name) => (service_handle: sf::MoveHandle))
    }

    fn register_service(&mut self, name: ServiceName, is_light: bool, max_sessions: i32) -> Result<sf::MoveHandle> {
        ipc_client_send_request_command!([self.session.session; 2] (name, is_light, max_sessions) => (port_handle: sf::MoveHandle))
    }

    fn atmosphere_install_mitm(&mut self, name: ServiceName) -> Result<(sf::MoveHandle, sf::MoveHandle)> {
        ipc_client_send_request_command!([self.session.session; 65000] (name) => (query_handle: sf::MoveHandle, port_handle: sf::MoveHandle))
    }

    fn atmosphere_uninstall_mitm(&mut self, name: ServiceName) -> Result<()> {
        ipc_client_send_request_command!([self.session.session; 65001] (name) => ())
    }
    
    fn atmosphere_acknowledge_mitm_session(&mut self, name: ServiceName) -> Result<(MitmProcessInfo, sf::MoveHandle)> {
        ipc_client_send_request_command!([self.session.session; 65003] (name) => (info: MitmProcessInfo, session_handle: sf::MoveHandle))
    }

    fn atmosphere_has_service(&mut self, name: ServiceName) -> Result<bool> {
        ipc_client_send_request_command!([self.session.session; 65100] (name) => (has: bool))
    }
}

impl server::IServer for UserInterface {
